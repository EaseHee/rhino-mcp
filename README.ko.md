...
- **Standalone(독립 실행)**: [`rhino3dm`](https://github.com/mcneel/rhino3dm) 라이브러리로 헤드리스 `.3dm` 파일 직접 작성. Rhino 미설치 환경에서도 동작, **약 234개 도구** 노출(구성 단축, 도큐먼트 위생, 지오메트리 검증, GH 템플릿 카탈로그, 비정형 skin / 패널 합리화 / 곡률 / 필드, **도면 시트 + 표제란, 면적 / 자재 산출, 블록 정의, 일조 위치 + 그림자 투영, IFC 메타 태깅, 19종 물리 머티리얼 프리셋, BRE 주광율 + Bird DNI 청천공 모델** 포함).
