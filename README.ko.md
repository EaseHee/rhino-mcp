...
- **Standalone(독립 실행)**: [`rhino3dm`](https://github.com/mcneel/rhino3dm) 라이브러리로 헤드리스 `.3dm` 파일 직접 작성. Rhino 미설치 환경에서도 동작, **약 129개 도구** 노출(v0.3 기준 — 구성 단축, 도큐먼트 위생, 지오메트리 검증, GH 템플릿 카탈로그, 비정형 skin / 패널 합리화 / 곡률 / 필드, **도면 시트 + 표제란, 면적 / 자재 산출, 블록 정의, 일조 위치 + 그림자 투영, IFC 메타 태깅, 19종 물리 머티리얼 프리셋, BRE 주광율 + Bird DNI 청천공 모델** 포함).
- **Bridge(브리지)**: Rhino 8 안에서 C# 브리지 플러그인 로드 시 모든 호출(부울 연산, 로프트, 스윕, 뷰포트, 렌더, 스크립트 실행, 변형, NURBS 편집, SubD, 패널링, 정확한 가우스 곡률을 포함한 비정형 분석, **다중 뷰 도면 배치 / 단면 / PDF 내보내기, AreaMassProperties 기반 Brep 면적 산출, 블록 폭파·재정의, ray-cast 일조 노출, IFC / gbXML 입출력, HDRI 환경, 카메라 / 라이트 / 렌더 / 턴테이블 자동화**, 모든 Grasshopper 명령 + 템플릿)을 라이브 Rhino로 투명 전달. **약 235개 도구** 활성.
...
235개 이상의 도구를 모두 사용하려면 C# 플러그인 빌드 후 설치.
