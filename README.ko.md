...
브리지 플러그인 로드 상태에서 Rhino 명령창에 `_McpInstall` 입력 → 동일 설치 파이프라인 자동 실행. 비대화형(non-interactive). `PATH` 의 `uvx` / `rhino-mcp` / `python` 탐색 (macOS GUI 앱 `PATH` 가 빈약한 점 보완 위해 `~/.local/bin`, `~/.cargo/bin`, `/opt/homebrew/bin`, `/usr/local/bin` 추가 검색), 결과는 Rhino 명령창으로 스트리밍.  — Claude Desktop 실행 시점에 Rhino 가 떠 있으면 브리지 자동 연결, 꺼져 있으면 standalone(rhino3dm) 자동 폴백.
