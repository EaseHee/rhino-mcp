...
항목별 PASS / FAIL / WARN 결과와 수정 방법 출력.
...
"Bridge responded to ping" 메시지와 함께 Rhino/GH 버전 출력 시 정상.
...
Claude Desktop에서 JSON 디코드 오류 발생 시 stdout으로 출력이 새는 상태.

```bash
RHINO_MCP_LOG_LEVEL=DEBUG uv run rhino-mcp  # 모든 로그가 stderr인지 확인
```

`rhino-mcp`는 stdout을 MCP 프레임 전용으로 예약, 로그는 항상 stderr로 출력.

### 다중 Rhino 인스턴스 — 의도한 문서가 응답 안 함 (v0.6)

증상: 도구가 *다른* Rhino 도큐먼트를 편집, 또는
`rhino_bridge_list_instances`가 2개 이상 떠 있는데 0/1개만 반환.

점검:

1. `RHINO_MCP_LISTENER_DIR` 디렉토리에서 listener 파일 확인.
   기본값: POSIX `${TMPDIR:-/tmp}/rhino-mcp-listeners`,
   Windows `%LOCALAPPDATA%/rhino-mcp/listeners`.

   ```bash
   ls /tmp/rhino-mcp-listeners/
   ```

   인스턴스당 `{pid}-{port}.json` 1개 기대.

2. 2개 Rhino 중 1개만 파일이 보이면, 그 Rhino는 v0.6 이전 플러그인 빌드
   (announcement writer 미포함). 재빌드 + 재로드:
...
4. stale 항목 정리 (Rhino crash로 청소 못한 경우): 다음
   `rhino_bridge_list_instances` 호출 시 `stale_cleanup=true`(기본)로
   PID 사망한 JSON 자동 삭제.

### `rhino_execute_csharp` 실행 전 거절

에러 메시지에 `C# execution is disabled. Set RHINO_MCP_ALLOW_CSHARP=1`
포함. opt-in 환경변수 게이트 작동 중. **Rhino 프로세스 측**(또는
브리지 플러그인 환경) 환경변수 설정 후 재기동:

```bash
RHINO_MCP_ALLOW_CSHARP=1 /Applications/Rhino\ 8.app/Contents/MacOS/Rhinoceros
```

게이트는 의도적 — Roslyn 스크립트는 RhinoCommon 전체 접근 권한 보유.
